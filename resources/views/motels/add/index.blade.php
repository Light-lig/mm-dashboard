...
<!-- Styles -->
<!-- Scripts -->
<script src="{{ asset('js/views/motels/add/add.js')}}" type="module"></script>
@endpush
@section('content')
<div class="container">
    <div class="row justify-content-center">
        <h1 class="display-4">Agregar motel</h1>

        <div class="col-md-8">
            <a class="btn btn-primary w-25 mb-2" href="{{route('admin.motels.index')}}" role="button">Volver</a>

            <form action="{{route('admin.motels.store')}}" method="POST" enctype="multipart/form-data">
              @csrf

                <div class="mb-3">
                  <label for="mo_nombre" class="form-label">Nombre</label>
                  <input type="text" class="form-control" id="mo_nombre" name="mo_nombre" aria-describedby="emailHelp" required>
                </div>
                ...
                <div class="mb-3">
                  ...
                  <input type="file" class="form-control" id="mo_foto_portada" name="mo_foto_portada" required>
                </div>
                <div class="mb-3">
                  ...
                  <select name="cat_id" id="cat_id" class="form-select" aria-label="Default select example" required>
                    ...
                    <option value="{{$item['cat_id']}}">{{$item['cat_tipo']}}</option>

                    @endforeach
                  </select>
                </div>
                <div id="map"></div>
                <input type="hidden" name="mo_latitud" id="mo_latitud"/>
                <input type="hidden" name="mo_longitud" id="mo_longitud"/>
                <div class="mb-3">
                    <label for="mo_direccion" class="form-label">Direccion</label>
                    <textarea class="form-control" id="mo_direccion" rows="3" name="mo_direccion" required></textarea>
                  </div>
                  <div class="mb-3">
                    ...
                    <select name="dep_id" id="dep_id" class="form-select" aria-label="Default select example" required>
                      <option selected value="">Seleccione un departemento</option>
                    
                    </select>
                  </div>
                  <div class="mb-3">
                    <label for="mun_id" class="form-label">Municipio</label>
                    <select id="mun_id" name="mun_id" class="form-select" aria-label="Default select example" required>
